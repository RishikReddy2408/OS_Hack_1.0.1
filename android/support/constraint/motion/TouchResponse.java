...
class TouchResponse
{
  ...
  static
  {
    float[] arrayOfFloat = { 1.0F, 0.5F };
    TOUCH_SIDES = new float[][] { { 0.5F, 0.0F }, { 0.0F, 0.5F }, arrayOfFloat, { 0.5F, 1.0F }, { 0.5F, 0.5F } };
  }
  ...
  void processTouchEvent(MotionEvent paramMotionEvent, MotionScene paramMotionScene)
  {
    ...
    case 1: 
      ...
      float f5 = mVelocityTracker.getYVelocity();
      ...
      {
        f6 = Math.min(mMotionLayout.getWidth(), mMotionLayout.getHeight());
        paramMotionEvent = mAnchorDpDt;
        mAnchorDpDt[0] = f6;
        paramMotionEvent[1] = f6;
      }
      float f6 = mAnchorDpDt[0];
      f6 = mAnchorDpDt[1];
      if (mTouchDirectionX != 0.0F) {
        f2 = f2 * mTouchDirectionX / mAnchorDpDt[0];
      } else {
        f2 = f5 * mTouchDirectionY / mAnchorDpDt[1];
      }
      if (!Float.isNaN(f2)) {
        f1 = f4 + f2 / 3.0F;
      }
      if ((f1 != 0.0F) && (f1 != 1.0F))
      {
        ...
        paramMotionEvent.touchAnimateTo(f3, f2);
      }
      mVelocityTracker.recycle();
      mVelocityTracker = null;
      return;
  ...
  void scrollUp(float paramFloat1, float paramFloat2)
  {
    ...
    if (f1 != 0.0F)
    {
      ...
      if (f1 != 1.0F)
      {
        ...
        localMotionLayout.touchAnimateTo(paramFloat2, paramFloat1);
      }
    }
  }
